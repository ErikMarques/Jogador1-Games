...
const authStyles = {
  screen: {
    minHeight: "100vh",
    background:
      "radial-gradient(circle at top left, rgba(220, 38, 38, 0.22), transparent 36%), #050505",
    display: "flex",
  ...
  card: {
    ...
    borderRadius: 28,
    padding: 36,
    boxShadow: "0 0 80px rgba(220,38,38,0.18)",
    display: "flex",
...
export default function App() {
  ...
  async function signIn() {
    if (!supabase) {
      setAuthError("Supabase não configurado. Verifique as variáveis de ambiente na Vercel.");
      return;
    }
    ...
    return (
      <div style={authStyles.screen}>
        <div style={authStyles.card}>
          <h1 style={authStyles.title}>Jogador1 Games</h1>
          <p style={authStyles.subtitle}>Carregando acesso...</p>
        </div>
      </div>
    );
  }
      ...
      <div style={authStyles.screen}>
        <div style={authStyles.card}>
          ...
          <h1 style={authStyles.title}>
            JOGADOR<span style={{ color: "#ef4444" }}>1</span> GAMES
          </h1>
          <p style={authStyles.subtitle}>Sistema Premium de Estoque</p>
          ...
          <button style={authStyles.button} onClick={signIn}>
            Entrar
          </button>
        </div>
      </div>
