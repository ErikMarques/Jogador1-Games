...
function ExtraCostSelector({ extraCosts, selected, toggleExtraCost }) {
  return (
    <div className="extra-selector">
      <div className="extra-selector-title"><Plus size={16} /> Custos extras</div>
      {!extraCosts.length && <p className="muted">Cadastre custos no menu Custos Extras para usar aqui.</p>}
      <div className="extra-selector-list">
        {extraCosts.map((cost) => {
          const checked = selected.some((item) => (item.custo_extra_id || item.id) === cost.id);
          return <button key={cost.id} type="button" className={checked ? "extra-chip selected" : "extra-chip"} onClick={() => toggleExtraCost(cost)}>{cost.descricao} • {currency(cost.valor)}</button>;
        })}
      </div>
    </div>
  );
...
function CalendarModal({ events, onClose }) {
  return (
    <div className="modal-backdrop">
      <div className="version-modal calendar-modal">
        <div className="modal-header"><div><h2>Calendário de movimentações</h2><p>Compras e vendas registradas por dia.</p></div><button onClick={onClose}><X size={18} /> Fechar</button></div>
...
function VersionModal({ onClose }) {
  return <div className="modal-backdrop"><div className="version-modal"><div className="modal-header"><div><h2>Novidades da versão {APP_VERSION}</h2><p>Resumo da atualização.</p></div><button onClick={onClose}><X size={18} /> Fechar</button></div><ul><li>Produtos vendidos saem da listagem de estoque.</li><li>Custos extras agora são selecionáveis no cadastro do produto.</li><li>Calendário exibe mês atual automaticamente.</li><li>Popup de calendário mostra compras e vendas por dia.</li></ul></div></div>;
}
